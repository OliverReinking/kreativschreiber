            ...
            <JetFormSection @submitted="addTeamMember">
                ...
                <template #form>
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-layout-600">
                    ...
                    <div v-if="availableRoles.length > 0" class="col-span-6 lg:col-span-4">
                        ...
                        <div class="relative z-0 mt-1 border border-layout-200 rounded-lg cursor-pointer">
                            <button
                                ...
                                :class="{'border-t border-layout-200 rounded-t-none': i > 0, 'rounded-b-none': i != Object.keys(availableRoles).length - 1}"
                                @click="addTeamMemberForm.role = role.key"
                            >
                                <div :class="{'opacity-50': addTeamMemberForm.role && addTeamMemberForm.role != role.key}">
                                    <!-- Role Name -->
                                    <div class="flex items-center">
                                        <div class="text-sm text-layout-600" :class="{'font-semibold': addTeamMemberForm.role == role.key}">
                                            {{ role.name }}
                                        </div>
                                    ...
                                    <!-- Role Description -->
                                    <div class="mt-2 text-xs text-layout-600 text-left">
                                        {{ role.description }}
                                    </div>
                                </div>
                            </button>
                        ...
                        <div v-for="user in team.users" :key="user.id" class="flex items-center justify-between">
                            ...
                            <div class="flex items-center">
                                ...
                                <button
                                    ...
                                    class="ml-2 text-sm text-layout-400 underline"
                                    @click="manageRole(user)"
                                >
                                    {{ displayableRole(user.membership.role) }}
                                </button>

                                <div v-else-if="availableRoles.length" class="ml-2 text-sm text-layout-400">
                                    {{ displayableRole(user.membership.role) }}
                                </div>

                                <!-- Leave Team -->
                                <button
                                    v-if="$page.props.userdata.user_id === user.id"
                    ...
                    <div class="relative z-0 mt-1 border border-layout-200 rounded-lg cursor-pointer">
                        ...
                        >
                            <div :class="{'opacity-50': updateRoleForm.role && updateRoleForm.role !== role.key}">
                                <!-- Role Name -->
                                <div class="flex items-center">
                                    <div class="text-sm text-layout-600" :class="{'font-semibold': updateRoleForm.role === role.key}">
                                        {{ role.name }}
                                    </div>
                                ...
                                <!-- Role Description -->
                                <div class="mt-2 text-xs text-layout-600">
                                    {{ role.description }}
                                </div>
                            </div>
                        </button>
